...
function p.LoadUI()
	...
	layer:Init();
	layer:SetTag(NMAINSCENECHILDTAG.LoginBindAccountUI);
...
function p.OnUIEvent(uiNode, uiEventType, param)
	...
	if uiEventType == NUIEventType.TE_TOUCH_BTN_CLICK then
		if ID_BTN_BIND == tag then     --绑定账号
			...
			if not LoginCommon.CheckDataValidity(p.Account, LoginCommon.CHECK_FLAG.TYPE_BIND_ACNT) then
				return true;
			end
			
			--密码基本校验
			p.FstPassWord = LoginCommon.GetEditData(p.CurLayer, ID_EDIT_FST_PASSWORD);
			if not LoginCommon.CheckDataValidity(p.FstPassWord, LoginCommon.CHECK_FLAG.TYPE_BIND_PWD_FST) then
				return true;
			end
			...
			if p.FstPassWord ~= p.SedPassWord then
				LoginCommon.ShowErrorTipInfo(LoginCommon.CHECK_FLAG.TYPE_BIND_PWD_SND, true);
				return true;
			end
			...
			if record ~= nil then
				--发起绑定请求
				SendHttpMsgLogin.HttpSendBindAccount(record.Account, p.FstPassWord, p.Account);
			end
